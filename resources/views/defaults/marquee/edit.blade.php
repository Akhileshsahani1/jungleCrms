...
@section('content')
    ...
    <section class="content">
        <div class="card">
            ...
            <div class="card-body">
                <form method="post" id="weekendForm" action="{{ route('marquees.update', $marquee->id) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                name="content" value="{{ old('content', isset($marquee) ? $marquee->content: '') }}">
                                @error('content')
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                name="text_color" value="{{ old('text_color', isset($marquee) ? $marquee->text_color: '') }}">
                                @error('text_color')
                                    <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                name="background_color" value="{{ old('background_color', isset($marquee) ? $marquee->background_color : '' ) }}">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <select id="statuses" class="form-control" name="status">
                                    <option value="1" {{ old('status', $marquee->status) == 1 ? "selected" : "" }}>Active</option>
                                    <option value="0" {{ old('status', $marquee->status) == 0 ? "selected" : "" }}>Inactive</option>
                                </select>
                                @error('status')
                                    <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="role" name="roles[]" multiple="multiple">
                                    @php 
                                        $roleids = explode(",", $marquee->roles);
                                    @endphp
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}" <?php if(in_array($role->id, $roleids)) echo "selected"; else echo "";  ?>>{{ $role->name }}</option>
                                        @endforeach
                                </select>
                                @error('roles')
                                    <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
