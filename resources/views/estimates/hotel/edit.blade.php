...
@section('title', 'Edit Hotel Estimate')
@section('head')
...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><i class="fas fa-hotel"></i> Edit Hotel Estimate</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('estimates.index') }}">Estimates</a></li>
                        <li class="breadcrumb-item active">Edit Hotel Estimate</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <section class="content">
        <div class="card">
            ...
            <form method="POST" action="{{ route('hotel-estimate.update', $estimate->id) }}" id="hotelForm">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
    ...
    @isset($estimate)
        <script>
           $(document).ready(function () {
            ...
            switch(inclusion_filter) {
            case 'normal':
                $('.inclusion-weekend').hide();
                $('.inclusion-festival').hide();
                $('.inclusion-normal').show();
                break;
            case 'weekend':
                $('.inclusion-festival').hide();
                $('.inclusion-normal').hide();
                $('.inclusion-weekend').show();
                break;
            case 'festival':
                $('.inclusion-weekend').hide();
                $('.inclusion-normal').hide();
                $('.inclusion-festival').show();
                break;
            default:
                $('.inclusion-weekend').hide();
                $('.inclusion-festival').hide();
        ...
        var exclusion_filter = $("#exclusion_filter").val();
            switch(exclusion_filter) {
            case 'normal':
                $('.exclusion-weekend').hide();
                $('.exclusion-festival').hide();
                $('.exclusion-normal').show();
                break;
            case 'weekend':
                $('.exclusion-festival').hide();
                $('.exclusion-normal').hide();
                $('.exclusion-weekend').show();
                break;
            case 'festival':
                $('.exclusion-weekend').hide();
                $('.exclusion-normal').hide();
                $('.exclusion-festival').show();
                break;
            default:
                $('.exclusion-weekend').hide();
                $('.exclusion-festival').hide();
    ...
    function addHotelOption() {
        if (hotel_option_row < 50) {
            ...
            html += '<select class="form-control hotel_id" name="option[' + hotel_option_row +
                '][hotel_id]" id="hotel_id' + hotel_option_row + '" onchange="getRooms(' + hotel_option_row + ', this.value)">';
            ...
            html += '<select class="form-control service" name="option[' + hotel_option_row +
                '][service_id]" id="service_id' + hotel_option_row + '" onchange="getTotal(' + hotel_option_row + ', this.value)">';
            ...
            html += '<button type="button" class="btn bg-success" id="amount-button'+ hotel_option_row +'" style="display: none;" onclick="openHotelModal('+ hotel_option_row +')">Total</button>';
            html += '<input type="hidden" class="btn bg-grey" id="amount'+ hotel_option_row +'" name="option['+ hotel_option_row +'][amount]">';
            html += '</td>';
