@if (in_array('cab', $estimate_type) && count($estimate->cabs) > 0)
                ...
                <div class="row">
                    <div class="form-group col-sm-4">
                        ...
                        <input type="text" id="trip_type0" class="form-control cab" placeholder="Trip Type"
                            ...
                            value="{{ in_array('cab', $estimate_type) ? $cab->trip_type : '' }}" required>
                    </div>                    
                    <div class="form-group col-sm-4">
                        ...
                        <select type="text" class="form-control cab" id="cab_type0"
                            ...
                            <optgroup label="Main">
                                <option value="Jeep" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Jeep') selected @endif>Jeep</option>
                                <option value="Canter"  @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Canter') selected @endif>Canter</option>
                            </optgroup>
                            <optgroup label="Hatchback">
                                <option value="Maruti Suzuki WagonR" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Maruti Suzuki WagonR') selected @endif>
                                    Maruti Suzuki WagonR</option>
                                <option value="Maruti Suzuki Swift" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Maruti Suzuki Swift') selected @endif>
                                    Maruti Suzuki Swift</option>
                                <option value="Maruti Suzuki Celerio" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Maruti Suzuki Celerio') selected @endif>
                                    Maruti Suzuki Celerio</option>
                            </optgroup>
                            <optgroup label="MPV">
                                <option value="Maruti Suzuki Eeco" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Maruti Suzuki Eeco') selected @endif>
                                    Maruti Suzuki Eeco</option>
                                <option value="Maruti Suzuki Ertiga" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Maruti Suzuki Ertiga') selected @endif>
                                    Maruti Suzuki Ertiga</option>
                                <option value="Maruti Suzuki XL6" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Maruti Suzuki XL6') selected @endif>
                                    Maruti Suzuki XL6</option>
                                <option value="Toyota Innova Crysta"
                                    @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Toyota Innova Crysta') selected @endif>
                                    Toyota Innova Crysta</option>
                                <option value="Innova" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Innova') selected @endif>
                                    Innova</option>
                            </optgroup>
                            <optgroup label="Sedan">
                                <option value="Swift Dezire" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Swift Dezire') selected @endif>
                                    Swift Dezire</option>
                                <option value="Toyota Etios" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Toyota Etios') selected @endif>
                                    Toyota Etios</option>
                            </optgroup>
                            <optgroup label="SUV">
                                <option value="Tata Safari" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Tata Safari') selected @endif>
                                    Tata Safari</option>
                                <option value="Mahindra Scorpio" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Mahindra Scorpio') selected @endif>
                                    Mahindra Scorpio</option>
                                <option value="Hyundai Creta" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Hyundai Creta') selected @endif>
                                    Hyundai Creta</option>
                                <option value="Jeep" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Jeep') selected @endif>
                                    Jeep</option>
                            </optgroup>
                            <optgroup label="Traveller">
                                <option value="Force Traveller" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Force Traveller') selected @endif>
                                    Force Traveller</option>
                                <option value="Mini Bus" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Mini Bus') selected @endif>
                                    Mini Bus</option>
                                <option value="Canter" @if (in_array('cab', $estimate_type) && $cab->vehicle_type == 'Canter') selected @endif>
                                    Canter</option>
                            </optgroup>
                        </select>
                    </div>
                    <div class="form-group col-sm-4">
                        ...
                        <input type="date" id="start_date0" class="form-control cab" placeholder="Journey Start Date"
                            ...
                            value="{{ in_array('cab', $estimate_type) && isset($cab->start_date) ? $cab->start_date : '' }}" required>
                    </div>
                    <div class="form-group col-sm-4">
                        ...
                        <input type="date" id="end_date0" class="form-control cab" placeholder="Journey End Date"
                            ...
                            value="{{ in_array('cab', $estimate_type) && isset($cab->end_date) ? $cab->end_date : '' }}" required>
                    </div>
                    <div class="form-group col-sm-4">
                        <label for="days0">No of Days</label>
                        <input type="number" id="days0" class="form-control cab" placeholder="No of Days"
                            name="trip[{{ $key }}][days]"
                            value="{{ in_array('cab', $estimate_type) && isset($cab->days) ? $cab->days : '' }}" required>
                    </div>
                    <div class="form-group col-sm-4">
                        ...
                        <input type="text" id="pick_up0" class="form-control cab" placeholder="Pickup Point"
                            ...
                            value="{{ in_array('cab', $estimate_type) && isset($cab->pick_up) ? $cab->pick_up : '' }}" required>
                    </div>
                    <div class="form-group col-sm-4">
                        <label for="drop0">Drop Point</label>
                        <input type="text" id="drop0" class="form-control cab" placeholder="Drop Point"
                            name="trip[{{ $key }}][drop]"
                            value="{{ in_array('cab', $estimate_type) && isset($cab->drop) ? $cab->drop : '' }}" required>
                    </div>
                    <div class="form-group col-sm-4">
                        ...
                        <input type="time" id="pickup_time0" class="form-control cab" placeholder="Pickup Time"
                            ...
                            value="{{ in_array('cab', $estimate_type) && isset($cab->pickup_time) ? $cab->pickup_time : '' }}" required>
                    </div>
                    <div class="form-group col-sm-4">
                        ...
                        <input type="number" id="total_riders0" class="form-control cab" placeholder="No of Riders"
                            ...
                            value="{{ in_array('cab', $estimate_type) && isset($cab->total_riders) ? $cab->total_riders : '' }}" required>
                    </div>
                    <div class="form-group col-sm-12">
                        ...
                        <input type="number" id="no_of_cab0" class="form-control cab" placeholder="No. of Cab"
                            ...
                            value="{{ in_array('cab', $estimate_type) && isset($cab->no_of_cab) ? $cab->no_of_cab : 1 }}" required>
                    </div>
                    <div class="form-group col-sm-12">
                        <label for="cab_note0">Cab Note</label>
                        <textarea class="form-control cab summernote" id="cab_note0" name="trip[{{ $key }}][cab_note]">{{ in_array('cab', $estimate_type) && isset($cab->note) ? $cab->note : '' }}</textarea>
                    </div>
                </div>
            </div>
        </div>
