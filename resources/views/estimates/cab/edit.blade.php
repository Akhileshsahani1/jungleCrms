...
@section('content')
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1><i class="fas fa-taxi"></i> Edit Estimate</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="card">
            ...
            <form method="POST" action="{{ route('cab-estimate.update', $estimate->id) }}" id="cabForm">
                @csrf
                @method('PUT')
                <input type="hidden" value="{{ $estimate->lead_id }}" name="lead_id">
                <div class="card-body">
                    <div class="row">
...
@push('scripts')
    ...
    <script>
        ...
        function addCabOption() {
            if (cab_option_row < 50) {
                ...
                html += '<td style="width:350px"><input type="text" name="option[' + cab_option_row +
                    '][content]" placeholder="Content" class="form-control" id="content' + cab_option_row +
                    '" required></td>';
                html += '<td><input type="number" name="option[' + cab_option_row +
                    '][amount]" placeholder="Amount" class="form-control amount" id="amount' + cab_option_row +
                    '" value="0" required></td>';
                html += '<td><input type="number" name="option[' + cab_option_row +
                    '][discount]" placeholder="Discount" class="form-control discount" id="discount' + cab_option_row +
                    '" value="0" required></td>';
                html += '<td class="text-right"><button type="button" onclick="$(\'#cab-option' + cab_option_row +
    ...
    <script>
       @if (isset($estimate) && count($estimate->cab->halts) > 0)
...
<script>
    $(document).ready(function () {
        getIternaries();
        var destIternary = '{{ $estimate->iternary }}'
        console.log(destIternary);
        // $('#iternary').val(destIternary);
