            ...
            <div class="row">
                <div class="form-group col-sm-12">
                    ...
                    <select id="sanctuary" class="form-control" name="sanctuary" onchange="loadForm()">
                        ...
                        <option value="gir"
                            {{ old('sanctuary', isset($booking) ? $booking->safari->sanctuary : '') == 'gir' ? 'selected' : '' }}>
                            Gir</option>
                        <option value="jim"
                            {{ old('sanctuary', isset($booking) ? $booking->safari->sanctuary : '') == 'jim' ? 'selected' : '' }}>
                            Jim Corbett</option>
                        <option value="ranthambore"
                            {{ old('sanctuary', isset($booking) ? $booking->safari->sanctuary : '') == 'ranthambore' ? 'selected' : '' }}>
                            Ranthambore</option>

                        <option value="tadoba"
                        {{ old('sanctuary', isset($booking) ? $booking->safari->sanctuary : '') == 'tadoba' ? 'selected' : '' }}>
                        Tadoba</option>
                    </select>
                    @error('sanctuary')
            ...
            <div class="row">
                ...
                <div class="form-group col-sm-6">
                    ...
                    <input type="number" id="amount" class="form-control" placeholder="Total Amount" name="amount"
                        value="{{ old('amount', isset($booking) ? $booking->option_selected->total : '') ? $booking->option_selected->total : '0' }}">
                    @error('amount')
                        <span id="name-error" class="error invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group col-sm-12">
                    ...
                    <select name="website" id="websites" class="form-control">
                        ...
                        <option value="ranthamboretigerreserve.in"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'ranthamboretigerreserve.in' ? 'selected' : '' }}>
                            ranthamboretigerreserve.in</option>
                        <option value="jimcorbettnationalparkonline.in"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'jimcorbettnationalparkonline.in' ? 'selected' : '' }}>
                            jimcorbettnationalparkonline.in</option>
                        <option value="girsafaribooking.com"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'girsafaribooking.com' ? 'selected' : '' }}>
                            girsafaribooking.com</option>
                        <option value="jimcorbett.in"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'jimcorbett.in' ? 'selected' : '' }}>
                            jimcorbett.in</option>
                        <option value="girlionsafari.com"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'girlionsafari.com' ? 'selected' : '' }}>
                            girlionsafari.com</option>
                        <option value="girlion.in"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'girlion.in' ? 'selected' : '' }}>
                            girlion.in</option>
                        <option value="bandhavgarh.com"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'bandhavgarh.com' ? 'selected' : '' }}>
                            bandhavgarh.com</option>
                        <option value="travelwalacab.com"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'travelwalacab.com' ? 'selected' : '' }}>
                            travelwalacab.com</option>
                        <option value="dailytourandtravel.com"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'dailytourandtravel.com' ? 'selected' : '' }}>
                            dailytourandtravel.com</option>
                        <option value="internationaltrips.in"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'internationaltrips.in' ? 'selected' : '' }}>
                            Internationaltrips.in</option>
                         <option value="tadobapark.com"
                            {{ old('website', isset($booking) ? $booking->website : $booking->lead->website) == 'tadobapark.com' ? 'selected' : '' }}>
            ...
            <table id="option" class="table table-striped">
                ...
                <tbody>
                    ...
                    <tr id="item-option-row0">
                        <td style="width:350px"><input type="text" name="item[0][particular]" placeholder="Particular"
                                class="form-control" id="particular0" value="Taxable amount" required></td>
                        <td><input type="text" name="item[0][amount]" placeholder="Amount" class="form-control amount"
                                id="amount0"
                                value="{{ old('amount', isset($booking) ? $booking->option_selected->total : '') ? $booking->option_selected->total : '0' }}"
                                required></td>
            ...
            <table id="details" class="table table-condensed table-sm">
                ...
                <tbody>
                    <tr id="customer-option-row0">
                        <td><input type="text" name="customer[0][name]" placeholder="Name" class="form-control"
                                id="name0" required></td>
                        <td><input type="number" name="customer[0][age]" placeholder="Age" class="form-control"
                                id="age0" required></td>
                        <td>
                        ...
                        <td><input type="text" name="customer[0][idproof_no]" placeholder="Id No" class="form-control"
                                id="idproof_no0" required></td>
