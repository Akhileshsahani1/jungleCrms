<aside class="main-sidebar sidebar-dark-primary elevation-4">
    <!-- Brand Logo -->
    <a href="{{ route('dashboard') }}" class="brand-link">
      <img src="{{ asset('dist/img/logo-small.png') }}" alt="Jungle Safari India" class="brand-image img-circle elevation-3" style="opacity: .8">
      <span class="brand-text font-weight-light">Jungle Safari India</span>
    </a>

    <!-- Sidebar -->
    <div class="sidebar">
      <!-- Sidebar Menu -->
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->
          <li class="nav-item">
            <a href="{{ route('dashboard') }}" class="nav-link">
              <i class="nav-icon fas fa-tachometer-alt"></i>
              <p>
                Dashboard
              </p>
            </a>
          </li>          
          <li class="nav-item">
            <a href="{{ route('dashboard.estimates') }}" class="nav-link">
              <i class="nav-icon fas fa-calculator"></i>
              <p>
                Estimates
              </p>
            </a>
          </li>
          <li class="nav-item">
            <a href="{{ route('dashboard.bookings') }}" class="nav-link">
              <i class="nav-icon fas fa-check-circle"></i>
              <p>
                Bookings
              </p>
            </a>
          </li>

          <li class="nav-item">
            <a href="{{ route('dashboard.cancel-bookings') }}" class="nav-link">
              <i class="nav-icon fas fa-times"></i>
              <p>
               Cancelled Bookings
              </p>
            </a>
          </li>
         
          <li class="nav-item">
            <a href="{{ route('dashboard.invoices') }}" class="nav-link">
              <i class="nav-icon fas fa-file-invoice"></i>
              <p>
                Invoices
              </p>
            </a>
          </li>

          <li class="nav-item">
            <a href="{{ route('dashboard.supports.index') }}" class="nav-link">
              <i class="nav-icon fas fa-headset"></i>
              <p>
                Support
              </p>
            </a>
          </li>

        </ul>
      </nav>
      <!-- /.sidebar-menu -->
    </div>
    <!-- /.sidebar -->
  </aside>
