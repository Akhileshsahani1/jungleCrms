            ...
            <div class="card-body">
                <dl class="row">
                    ...
                    @if ($ticket->attachment)
                        ...
                        <dd class="col-sm-8"><a href="{{ asset('storage/uploads/customers/tickets/'.Auth::guard('customer')->user()->id.'/'.$ticket->attachment) }}" class="btn btn-sm btn-warning" download><i class="fas fa-download"></i></a></dd>
                    @endif
        ...
        <div class="card card-primary card-outline direct-chat direct-chat-primary">
                        ...
                        @if ($chat->sender == 'customer')
                            <div class="direct-chat-msg right">
                                <div class="direct-chat-infos clearfix">
                                    <span class="direct-chat-name float-right">{{ \App\Models\Customer::where('id', $chat->customer_id)->first()->name }}</span>
           ...
           @if ($ticket->status == 0)
            <div class="card-footer">
                <form action="{{ route('dashboard.supports.send-message', $ticket->id) }}" method="POST">
                    @csrf
                    @method('PUT')
